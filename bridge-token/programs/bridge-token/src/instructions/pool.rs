...
pub fn remove_liquidity(ctx: Context<PoolLiquidity>, amount: u64) -> Result<()> {
    ...
    let precision = 1000000000000u64; // Magnify 1e12 processing accuracy
    let user_stake_ratio = old_amount * precision / pool_account.total_staked_liquidity;
    let withdraw_ratio = amount * precision / old_amount;
    let mut staked_decrease = (pool_account.total_staked * user_stake_ratio) / precision;
